...
ParseDirective = Enum("ParseDirective", ["Finalized", "NextRow"])
...
class Importer(importer.ImporterProtocol):
    ...
    def init_cost_posting_by_cmb_txn(
        self, cmb_txn: CmbTransaction, dest_account: str
    ) -> data.Posting:
        local_currency = "RMB"
        cost_spec = None
        if (
            cmb_txn.value_delta_local != None
            and cmb_txn.value_delta != cmb_txn.value_delta_local
        ):
            local_currency = "SGD"
    ...
    def init_outgoing_posting_by_cmb_txn(self, cmb_txn: CmbTransaction) -> data.Posting:
        ...
        return data.Posting(
            self.fop_account_map[cmb_txn.fop],
    ...
    def init_postings_by_cmb_txn(
        self, cmb_txn: CmbTransaction, dest_account
    ) -> List[data.Posting]:
        return [
    ...
    def grocery(self, cmb_txn):
        if cmb_txn.is_withdraw and (
            re.search(r"NTUC", cmb_txn.description) != None
            or re.search(r"DOOKKI", cmb_txn.description) != None
            or re.search(r"WATSON", cmb_txn.description) != None
            or re.search(r"COLD STORAGE", cmb_txn.description) != None
            or re.search(r"Redmart", cmb_txn.description) != None
        ):
    ...
    def grab(self, cmb_txn):
        if (
            cmb_txn.is_withdraw
            and re.search(r"Grab Grab*", cmb_txn.description) != None
            and cmb_txn.value_delta > Decimal(400)
            and cmb_txn.value_delta < Decimal(600)
        ):
            cost_posting = self.init_cost_posting_by_cmb_txn(
                cmb_txn, "Assets:Wallet:Grab"
            )
        ...
        if (
            ...
            return self.init_postings_by_cmb_txn(cmb_txn, "Expenses:Transit:Taxi")

    def cashback(self, cmb_txn):
        if not cmb_txn.is_withdraw and (
            re.search(r"返现", cmb_txn.description) != None
            or re.search(r"红包", cmb_txn.description) != None
        ):
    ...
    def utility(self, cmb_txn):
        if (
            cmb_txn.is_withdraw
            and re.search(r"SP DIGITAL", cmb_txn.description) != None
        ):
